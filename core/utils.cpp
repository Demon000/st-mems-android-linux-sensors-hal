...
int device_iio_utils::get_device_by_name(const char *name)
{
    ...
    for (ent = readdir(dp); ent; ent = readdir(dp)) {
        if (strlen(ent->d_name) <= strlen(device_iio_device_name) || strstr(ent->d_name, ".")) {
            continue;
        }

        if (strncmp(ent->d_name, device_iio_device_name, strlen(device_iio_device_name)) == 0) {
            numstrlen = sscanf(ent->d_name + strlen(device_iio_device_name), "%d", &number);
            fnamelen = numstrlen + strlen(device_iio_dir) + strlen(device_iio_device_name);
            if (fnamelen > DEVICE_IIO_MAX_FILENAME_LEN) {
                continue;
            }

            sprintf(dfilename, "%s%s%d/name", device_iio_dir, device_iio_device_name, number);
                                       ...
                                       unsigned int max_list)
{
    ...
    while (ent = readdir(dp), ent != NULL) {
        if (!strstr(ent->d_name, ".") && (strlen(ent->d_name) > iio_len) &&
            ...
            if (strncmp(ent->d_name + iio_len + numstrlen, ":", 1) != 0) {
                string_len = strlen(device_iio_dir) + iio_len + numstrlen + 6;
                                   ...
                                   int *counter)
{
    ...
    *data = NULL;
    ...
    snprintf(dname, sizeof(dname), "%s/scan_elements",
         device_dir);
    ...
    *counter = 0;
    ...
    for (ent = readdir(dp); ent; ent = readdir(dp))  {
        tmp = &ent->d_name[strlen(ent->d_name) - strlen(device_iio_scan_elements_en)];
        if (strncmp(tmp, device_iio_scan_elements_en, strlen(device_iio_scan_elements_en)) == 0) {
            if (strlen(dname) + strlen(ent->d_name) + 1 > DEVICE_IIO_MAX_FILENAME_LEN) {
                ret = -ENOMEM;
                goto error_cleanup_array;
            }

            /* open all scan_element xxx_en files and enable it */
            snprintf(dfilename, sizeof(dfilename), "%s/%s", dname, ent->d_name);
            ...
            if (ret > 0 && temp == 1) {
                ...
                *data = (struct device_iio_info_channel *)realloc(*data,
                ...
                snprintf(dfilename, sizeof(dfilename), "%s/%s_index", dname, current->name);
                sysfs_read_uint(dfilename, &current->index);

                sprintf(dfilename,  "%s/%s_scale", device_dir, current->name);
                ret = sysfs_read_float(dfilename, &current->scale);
                if (!ret) {
                    goto error_cleanup_array;
                }

                snprintf(dfilename, sizeof(dfilename), "%s/%s_offset", device_dir, current->name);
...
int device_iio_utils::enable_events(const char *device_dir, bool enable)
{
    ...
    err = snprintf(event_el_dir, sizeof(event_el_dir), device_iio_event_dir, device_dir);
    if (err < 0) {
        return err;
    }
    ...
    while (ent = readdir(dp), ent != NULL) {
        if (!strcmp(ent->d_name + strlen(ent->d_name) - strlen("_en"), "_en")) {
            err = snprintf(filename, sizeof(filename), "%s/%s", event_el_dir, ent->d_name);
            if (err < 0) {
                err = -ENOMEM;
                                                       ...
                                                       struct device_iio_sampling_freqs *sfa)
{
    ...
    err = sprintf(sf_filaname, "%s/%s", device_dir, device_iio_sfa_filename);
    if (err < 0) {
        return err;
    }
...
int device_iio_utils::get_hw_fifo_length(const char *device_dir)
{
    ...
    if (ret < 0) {
        console.error(std::string("Failed to enable hw fifo: ") + tmp_filaname);
    }
                                            ...
                                            unsigned int delay)
{
    ...
    ret = snprintf(tmp_filaname, DEVICE_IIO_MAX_FILENAME_LEN, "%s/%s", device_dir,
                               ...
                               const char *pre_name)
{
    ...
    if (strlen(name) + strlen("_type") + 1 > DEVICE_IIO_MAX_FILENAME_LEN) {
        return -1;
    }
    ...
    while (ent = readdir(dp), ent != NULL) {
        if ((strcmp(type_name, ent->d_name) == 0) || (strcmp(name_pre_name, ent->d_name) == 0)) {
            snprintf(filename, sizeof(filename), "%s/%s", dir, ent->d_name);
            sysfsfp = fopen(filename, "r");
            if (sysfsfp == NULL) {
                continue;
                                           ...
                                           device_iio_chan_type_t device_type)
{
    ...
    /* check string len */
    if (strlen(device_dir) + strlen(avl_name) + 1 > DEVICE_IIO_MAX_FILENAME_LEN) {
        return -ENOMEM;
    }
...
int device_iio_utils::support_injection_mode(const char *device_dir)
{
    ...
    /* Check string len */
    if (strlen(device_dir) + strlen(device_iio_injection_mode_enable) + 1 > DEVICE_IIO_MAX_FILENAME_LEN) {
        return -1;
    }

    sprintf(tmp_filaname, "%s/%s", device_dir, device_iio_injection_mode_enable);
    err = sysfs_read_int(tmp_filaname, &err);
    if (err < 0) {
        sprintf(tmp_filaname, "%s/%s", device_dir, device_iio_injection_sensors_filename);
        err = sysfs_read_str(tmp_filaname, injectors, sizeof(injectors));
        if (err < 0) {
            return err;
        }
                                  ...
                                  int len, device_iio_chan_type_t device_type)
{
    ...
    /* Check string len */
    if (strlen(device_dir) + strlen(injection_filename) + 1 > DEVICE_IIO_MAX_FILENAME_LEN) {
        return -1;
    }
...
int device_iio_utils::set_clock_type(const char *device_dir, const char *type)
{
    ...
    return sysfs_write_str((char *)tmp_filaname, (char *)type);
}
                                             ...
                                             char list[][20])
{
    ...
    err = sprintf(sf_filaname, "%s/%s", device_dir, device_iio_selftest_available_filename);
    if (err < 0) {
        return err;
    }
...
int device_iio_utils::execute_selftest(const char *device_dir, char *mode)
{
    ...
    err = sprintf(sf_filaname, "%s/%s", device_dir, device_iio_selftest_filename);
    if (err < 0) {
        return err;
    }
